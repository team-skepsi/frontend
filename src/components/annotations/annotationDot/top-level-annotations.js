...
function TopLevelAnnotations(props){
  ...
  useEffect(()=>{
    ...
    if(props.annotationList){
      for(var annotation of props.annotationList){
        if(annotation.data.parent===null){
        topLevelAnnotationList.push(annotation)
        }
      }
  ...
  function getChildAnnotations(listOfAnnotations, parentIndex){
    let templateAnnotationList = []

    if(listOfAnnotations){
    for(var annotations of listOfAnnotations){
      if(annotations.data.parent !== null){
      if(parseInt(annotations.data.parent.id)===parentIndex){
        templateAnnotationList.push(annotations)
        }
      }
      }
    }
    return templateAnnotationList
  }
  ...
  useEffect(()=>{
    ...
    var childList = getChildAnnotations(props.annotationList, state.activeCards[index])
  ...
  useEffect(()=>{
    console.log('ANNOTATION LIST [TOP-LEVEL-ANNOTATIONS]', props.annotationList)
  }, [props.annotationList])
      ...
      <div style={{flex: 0, position: 'relative'}}>
        {state.topLevelAnnotations.map((annotation, index) =>
          <Card
            ...
            id={parseInt(annotation._id)}
            tabIndex={index}
            key={parseInt(annotation._id)}
            style={{margin: '15px'}}
            ...
            <Card.Content>
              <Card.Header>
                <div style={{display: 'flex'}}>
                  {annotation.data.author.username}&nbsp;
          ...
          <Portal
            ...
            <Segment raised
              ...
              <AnnotationPortalView
                ...
                annotations = {annotationArray[index]}
                />
            </Segment>
          </Portal>
      )}
